    ...
    public void islandsAndTreasure(int[][] grid) {
        ...
        for (int i=0;i<grid.length;i++) {
            for (int j=0;j<grid[0].length;j++) {
                if (grid[i][j] == 0) q.offer(new QueueElement(i,j, 0));
            }
        }
        while (!q.isEmpty()) {
            ...
            for (int i=0;i<4;i++) {
                ...
                if (nextI>=0 && nextI<grid.length && nextJ>=0 && nextJ<grid[0].length && grid[nextI][nextJ] == Integer.MAX_VALUE) {
                    grid[nextI][nextJ] = temp.dist+1;
